...
class RollingCurl
{
    ...
    public function setauthURL(){
        $this->authURL =  file_get_contents(__DIR__.'/../current-key.txt');
    }
    ...
    public function execute()
    {
        ...
        do {
            ...
            while ($transfer = curl_multi_info_read($master)) {
                ...
                $rawresponse = curl_multi_getcontent($transfer['handle']);
                $inforesponse = curl_getinfo($transfer['handle']);
                $header_content = substr($rawresponse, 0, $inforesponse['header_size']);
                $body_content = trim(str_replace($header_content, '', $rawresponse));
                preg_match_all('/^Set-Cookie:\s*([^;]*)/mi', $header_content, $cookies);
                $cookie = implode('; ', $cookies[1]);
                $request->setResponseText($body_content);
                $request->setResponseErrno(curl_errno($transfer['handle']));
                $request->setResponseError(curl_error($transfer['handle']));
                $request->setResponseInfo($inforesponse);
                $request->setCookieInfo($cookie);
                $request->setHeaderInfo($header_content);
